...
import { Phone, MessageCircle, MapPin, Clock, Award, Factory, Truck, ShieldCheck } from 'lucide-react';
...
export default function ContactPage({ searchParams }: { searchParams: Search }) {
  ...
  return (
    <>
      {/* ---------------------------------------------------------------- opener */}
      <section className="border-b border-line bg-porcelain pt-24 md:pt-28">
        <div className="container-x grid items-center gap-10 pb-12 lg:grid-cols-[1fr_0.9fr] lg:gap-14 lg:pb-16">
          <div className="min-w-0">
            <Breadcrumbs items={[{ name: 'Home', href: '/' }, { name: 'Contact' }]} />
            <p className="mt-6 font-mono text-[11px] uppercase tracking-[0.18em] text-decart-700">
              We&rsquo;re here to help
            </p>
            <h1 className="mt-3 max-w-xl font-display text-h2 text-ink-950">Let&rsquo;s build your workspace</h1>
            <p className="mt-4 max-w-lg text-[0.9375rem] leading-relaxed text-steel-600">
              Talk straight to the people who make the furniture — one cabin or a full floor, dealer stock,
              OEM production and custom builds. No call-centre in between.
            </p>

            <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              <ButtonLink href={SITE.phoneHref} size="lg" className="w-full sm:w-auto">
            ...
            <p className="mt-4 flex items-center gap-2 text-xs text-steel-600">
              <Clock aria-hidden className="h-3.5 w-3.5 shrink-0" />
              {SITE.hours}
            </p>
          </div>

          <div className="relative aspect-[4/3] min-w-0 overflow-hidden rounded-img bg-decart-50 lg:aspect-[5/4]">
            <Image
              src="/families/table.webp"
              alt="A DecArt executive cabin — desk, storage and seating"
              fill
              priority
      ...
      <section className="bg-paper py-12 md:py-16">
        <div className="container-x grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:gap-12">
          <div className="min-w-0 rounded-card border border-line bg-paper p-5 md:p-7">
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-steel-400">Get in touch</p>
            <h2 className="mt-2 font-display text-h3 text-ink-950">Tell us what you&rsquo;re furnishing</h2>
            <p className="mt-2 text-[0.9375rem] text-steel-600">
              Going to the <strong className="font-semibold text-ink-950">{desk.title.toLowerCase()}</strong>{' '}
      ...
      {/* ---------------------------------------------------------------- factory */}
      <section className="border-y border-line bg-porcelain py-14 md:py-16">
        <div className="container-x grid gap-8 lg:grid-cols-[1fr_1.1fr_0.9fr] lg:items-center">
          <div className="min-w-0">
            <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-decart-700">Visit our factory</p>
            <h2 className="mt-2 font-display text-h3 text-ink-950">See where quality takes shape</h2>
            <p className="mt-3 text-[0.9375rem] leading-relaxed text-steel-600">
              Architects, dealers and buying teams are welcome on the floor. It is the shortest way to judge a
              manufacturer — you see the frames, the foam and the finish before you commit to a single chair.
            </p>
            <ButtonLink href={SITE.mapUrl} variant="secondary" external className="mt-6">
          ...
          <div className="flex min-w-0 flex-col gap-3">
            ...
            <div className="rounded-card border border-line bg-paper p-5">
              ...
              <p className="mt-2 text-xs text-steel-600">
                Call ahead and we will have somebody free who can answer your questions.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="container-x grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {PROOF.map((item) => (
            <div key={item.label} className="flex min-w-0 items-start gap-3">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-decart-50 text-decart-700">
                <item.icon aria-hidden className="h-4 w-4" />
              </span>
              <div className="min-w-0">
                <p className="text-[0.9375rem] font-semibold text-ink-950">{item.value}</p>
                <p className="mt-0.5 text-xs leading-snug text-steel-600">{item.label}</p>
              </div>
            </div>
          ))}
        </div>
