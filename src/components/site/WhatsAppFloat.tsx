...
/**
 * Floating enquiry buttons — every public page, bottom-right, above the sticky PDP action bar
 * on mobile.
 *
 * The button carries a visible "Ask price" label rather than a bare icon. Nobody in this market
 * publishes prices, so asking for one is the actual reason a visitor taps, and saying so out
 * loud converts better than a green circle. A call button sits under it for buyers who would
...
export function WhatsAppFloat() {
  ...
  return (
    <div
      className={cn(
        'fixed right-4 z-40 flex flex-col items-center gap-2 md:right-6 md:bottom-6',
    ...
    >
      <a
        href={SITE.phoneHref}
        data-call="float"
        aria-label={`Call DecArt on ${SITE.phone}`}
        className="flex h-11 w-11 items-center justify-center rounded-full bg-ink-900 text-porcelain shadow-lift transition-transform hover:scale-105 focus-visible:scale-105"
      >
      ...
      <a
        ...
        data-wa="float"
        aria-label="Ask DecArt for a price on WhatsApp"
        className={cn(
          'flex items-center gap-2 rounded-full bg-[#25D366] py-2.5 pl-2.5 pr-4 text-white shadow-lift animate-wa-pulse',
          'transition-transform hover:scale-105 focus-visible:scale-105',
        )}
      >
        ...
        <span className="text-sm font-semibold leading-none">Ask price</span>
      </a>
    </div>
  );
}
